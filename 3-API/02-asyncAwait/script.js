/*
- Introduced in ES8 async functions can be thought as an alternative way
    of writing promise based code- which allows us to adviod chaining promises

    -async/ await allows us to program using an asychronous request, like fetch in a synchronous manner
*/

//syntax: async function unlocks the use of the await keyword insise of the async function 
...
//async function are normal javascript functions but they alway retrun a promie by default
...
// The Fetch method is actually part of the browser window. When you run JS through nodeExec or code runner, it just runs it through the nodeJS or the JS engine, which doesn't have fetch as an object. If you attach the file to an HTML docume